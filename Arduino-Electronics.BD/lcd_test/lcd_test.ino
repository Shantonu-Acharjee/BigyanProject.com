...
#define PWM_PIN 9         // pin-3 is used to control the charging MOSFET //the default frequency is 490.20Hz
#define LOAD_PIN 2       // pin-2 is used to control the load
#define BAT_RED_LED 10
#define BAT_GREEN_LED 11
#define BAT_BLUE_LED 12
#define LOAD_RED_LED 13
#define LOAD_GREEN_LED 13
//--------------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------
 void read_data(void) 
 {
    ...
    // Vout=Vin*R2/(R1+R2) => Vin = Vout*(R1+R2)/R2   R1=100 and R2=20
     solar_volt = read_adc(SOL_ADC)*0.0048828;
     bat_volt   = read_adc(BAT_ADC)*0.0048828;       
     load_current = (read_adc(CURRENT_ADC)*0.048828-25); 
     temperature = read_adc(TEMP_ADC)*0.0048828*100;
...
void setpoint(void)
{
 ...
 // temperature compensation = -5mv/degC/Cell 
  ...
  else if(system_volt ==4)
  {
     bulk_charge_sp = (BULK_CH_SP/3)-(0.015*temp_change) ;
     float_charge_sp= (FLOAT_CH_SP/3)-(0.015*temp_change) ;
     lvd=LVD/3;
  }
...
void load_control()
{
 if (solar_volt < 5 && system_volt > 4 ) // load will on when night
  { 
  ...
  else if (solar_volt < 4 && system_volt == 4 ) // load will on when night
  { 
    if(bat_volt >lvd)   // check if battery is healthy 
    {
    load_status=1;
