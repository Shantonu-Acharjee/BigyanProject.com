...
const char website[] PROGMEM = "192.168.0.100/Final";
...
Password password = Password( "696969" );
Password username = Password( "696969" );
...
void setup() {
  ...
  ether.parseIp(ether.hisip, "192.168.0.100");
...
void loop() {
  ...
  if (int(key) != 0) {
    ...
    if (key == 'A')
    {
      ...
      else if (flag_user == 1 && flag_pass == 1 && flag_check == 0)
      {
        ...
        lcd.setCursor(0, 3);
        lcd.print("********************");
        sendToSweety(username.getGuess(),password.getGuess());
        //sendToSweety("1234","8888");
  ...
  delay(2000);
    ...
    else if (key == '0' || key == '1' || key == '2' || key == '3' || key == '4' || key == '5' || key == '6' || key == '7' || key == '8' || key == '9' || key == '*' || key == '#')
    {
...
void sendToSweety (char* userid,char* password) {
  ...
  Stash::prepare(PSTR("GET http://$F/udp.php?user_id=$S&password=$S HTTP/1.1" "\r\n"
                      "Host: $F" "\r\n"
                      "Content-Length: $D" "\r\n"
                      "\r\n"
                      "$H"),
