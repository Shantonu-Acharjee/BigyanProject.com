...
void loop() {
  ...
  if(message == "led on"){
    ...
    digitalWrite(alarmPin, HIGH); // Turn the LED on
  ...
  updateSerial(); //Update data for GSM
}//END vOID lOOP
...
void updateSerial() {
  ...
  while (GSM_800L.available()) {
    ...
    if (response.startsWith("+CMGR")) { // Check if the response starts with the expected SMS format
      ...
      GSM_800L.println("AT+CMGF=0"); //Delete all SMS
      delay(500); // Add a delay to ensure the command is processed
    }
  }
}
